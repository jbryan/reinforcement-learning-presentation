...
\begin{frame}
	\frametitle{Value of Policy}
		...
		policy $\pi$.
		\begin{eqnarray*}	
			V^\pi(s) &=& E[R(s_0) + \gamma (R(s_1) + \gamma( R(s_2) + \dots
			)) | \pi, s_0 = s]\\
			&=& E \Bigg[\sum_{t=0}^\infty \gamma^t R(s_t) \big| \pi, s_0 =
			s  \Bigg]
		\end{eqnarray*}	
...
\begin{frame}
	\frametitle{Alternative Value of Policy}
		...
		policy $\pi$.
		\begin{eqnarray*}	
			V^\pi(s) &=& \lim_{n \to \infty} E\Bigg[\frac{R(s_0) + R(s_1) + \dots +
			R(s_n)}{n} \Big| \pi, s_0 = s\Bigg]\\
			&=& \lim_{n \to \infty} E\Bigg[\frac{\sum_{t=0}^n
			R(s_n)}{n}\Big| \pi, s_0 = s\Bigg]
		\end{eqnarray*}	
\end{frame}
...
\begin{frame}
	...
	$V^{\pi^*}(s)$ for all $s$:
	\[
	\pi^* = \argmax_{\pi} E\Bigg[\sum_{t=0}^\infty \gamma^t R(s_t)\Big
	\pi\Bigg] 
	\]
	This implies:
	\[
	\pi^*(s) = \argmax_{a\in A} \sum_{s'\in S} T(s,a,s')U(s')
	\]
\end{frame}

...
\begin{frame}
	...
	\begin{block}{ As a running example, and as a demo at the end, we
		...
		\{0,\cdots,9\}^2$.  The set of actions are $\{up, down, left,
		right\}$.  30 of the grid squares are ``bad'' and give a reward
		of $-1$.  2 of the grid squares are ``good'' and give a reward of
		$1$.  Every action taken invokes a penalty of $-0.01$.
	\end{block}
\end{frame}
